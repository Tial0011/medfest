...
function renderFooter(mount) {
  ...
  const icons = {
    ...
    phone:
      '<path d="M8 3H4a1 1 0 0 0-1 1c0 9.4 7.6 17 17 17a1 1 0 0 0 1-1v-4l-5-2-2 2a15 15 0 0 1-6-6l2-2Z"/>',
  };
  const icon = (name) =>
    `<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${icons[name]}</svg>`;
  mount.innerHTML = `
        ...
        <div class="mf-footer__grid">
          <div class="mf-footer__brand">
            <a class="mf-footer__logo" href="${Utils.resolveHref(root, "index.html")}">
              <img src="${Utils.resolveHref(root, "assets/images/Logos/medfest-logo-dark.svg")}" alt="MedFest V — Home" width="395" height="211" loading="lazy" decoding="async">
            </a>
  ...
  mount.querySelector(".mf-footer__top").addEventListener("click", () => {
    window.scrollTo({
      top: 0,
      behavior: Utils.prefersReducedMotion() ? "instant" : "smooth",
    });
